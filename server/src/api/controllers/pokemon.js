...
export const generatePokemonSession = async (req, res) => {
    try {
...
export const getPokemonBatch = async (req, res) => {
    try {
        ...
        const startIndex = (batchNumber - 1) * 5;
...
export const checkPokemonResults = async (req, res) => {
    try {
        const { gameSessionId, guesses } = req.body;
        const session = await GameSession.findById(gameSessionId);
        if (!session) {
        ...
        const pokemonMap = pokemons.reduce((map, pokemon) => {
        ...
        for (const guess of guesses) {
            ...
            if (pokemon) {
                ...
                const keystrokeIntervals = guess.keystrokeIntervals || [];
                ...
                if (keystrokeIntervals.length > 0) {
                    ...
                    const unrealisticTotalTime = keystrokeIntervals.reduce((a, b) => a + b, 0) < 300;
