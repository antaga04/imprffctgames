...
const emailValidationMiddleware = async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!email) {
        return sendError(res, 400, {
            i18n: 'email.validation.required',
    ...
    try {
        ...
        if (result.error) {
            ...
            if (result.error.code === 104) {
                return sendError(res, 429, {
                    i18n: 'email.validation.limit_reached',
                    message: 'Maximum registrations reached for this month. Please try again later.',
                });
            }

            return sendError(res, 500, {
                i18n: 'email.validation.service_error',
                message: 'Email validation service error. Please try again later.',
            });
        }
        ...
        console.error('[emailValidationMiddleware] Error: ', error.message);
        return sendError(res, 500, {
            i18n: 'email.validation.failed',
